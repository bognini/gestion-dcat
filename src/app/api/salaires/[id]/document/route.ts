...
const UPLOAD_DIR = path.join(process.cwd(), 'public', 'uploads', 'salaires');
...
export async function POST(
  ...
  try {
    ...
    const filename = `salaire-${id}-${Date.now()}${ext}`;
    ...
    const salaire = await prisma.salaire.update({
      where: { id },
      data: {
        documentPath: `/uploads/salaires/${filename}`,
...
export async function DELETE(
  ...
  try {
    ...
    if (salaire?.documentPath) {
      const filepath = path.join(process.cwd(), 'public', salaire.documentPath);
      try {
